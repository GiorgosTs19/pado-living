...
const items = [
  ...
  // { title: 'Breakfast', icon: <div className="h-4 w-4">3</div>, href: '#Contact' },
];
...
const FloatingDockMobile = ({ items, className }: { items: { title: string; icon: ReactNode; href: string }[]; className?: string }) => {
...
const FloatingDockDesktop = ({ items, className }: { items: { title: string; icon: React.ReactNode; href: string }[]; className?: string }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'fixed top-5 left-[50%] -translate-x-1/2 min-w-md mx-auto hidden h-16 items-end gap-4 rounded-2xl bg-chill px-4 pb-3 md:flex dark:bg-neutral-900',
        className
      )}
    >
      {items.map(item => (
  ...
  return (
    <a href={href}>
      <motion.div
        ...
        className="relative flex aspect-square items-center justify-center rounded-full bg-chill"
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
